...
// returns 1 on successs and 0 on failure
extern DECLSPEC int KIWI_InitScheduler(const KIWI_SchedulerParams* params);

// shuts down worker threads and cleans up memory for scheduler
extern DECLSPEC void KIWI_FreeScheduler();
...
extern DECLSPEC void KIWI_SchedulerAddJob(const KIWI_Job* job, const KIWI_JobPriority priority, struct KIWI_Counter** counter);
...
extern DECLSPEC void KIWI_SchedulerAddJobs(const KIWI_Job* job, const int jobCount, const KIWI_JobPriority priority, struct KIWI_Counter** counter);
...
extern DECLSPEC void KIWI_SchedulerFreeCounter(struct KIWI_Counter* outCounter);
...
extern DECLSPEC void KIWI_SchedulerWaitForCounterAndFree(struct KIWI_Counter* counter, int targetValue);

// halts the job until counter reaches targetValue and then returns.
// this function WILL NOT free the counter, the programmer must free the counter
// NOTE: this function should not be called from outside of fibers
extern DECLSPEC void KIWI_SchedulerWaitForCounter(struct KIWI_Counter* counter, int targetValue);
